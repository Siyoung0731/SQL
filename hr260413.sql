...
create procedure GET_EMPSAL ( IN_EMPID IN NUMBER) -- 입력
IS
    V_NAME VARCHAR2(46);
    V_SAL NUMBER(8,2);
    BEGIN
        ...
        INTO V_NAME, V_SAL
        FROM EMPLOYEES
        WHERE EMPLOYEE_ID = IN_EMPID; -- EMPLOYEE_ID = 입력한 값
...
/
SET SERVEROUT ON;
CALL GET_EMPSAL(107); -- IN_EMPID = 107;
...
IS
  BEGIN 
    OPEN O_CUR FOR
        ...
        from departments
        where LOCATION_ID = IN_LCT;
  END;
/
create or replace procedure getemp_name (
    IN_LCT NUMBER,
    O_LCT NUMBER,
    O_NAME VARCHAR2
)
